...
# Fixed
# def do_model : Fixed bug -> umbrowser crashed when users typed wrong password in their login
# def lscontext : Show list with newline to see the list clearer / if ther is no components, it shows N/A
# change exception to updated syntax (except ValueError, e: -> except Exception as e:)
...
def lscontext(cont):
        ...
        try:
                ...
                print("Contexts:")
                print("%s" % str(contexts))
                print("Views:")
                print("%s" % str(theviews))
                print("Subscriptions:")
                print("%s" % str(thesubs))
...
class browse(cmd.Cmd):
        ...
        def do_tell(self, line):
                ...
                flags = []
                while True:
                        flag = input("Evidence flag? (CR for none)")
                        if flag == '':
                                break
                        flags.append(flag)
        ...
        def do_importmdef(self, line):
                """importmdef fromfilename
                        ...
                        imports into current active model"""
                global um, context, attrs
        ...
        def do_app(self, line):
                ...
                for item in requests:
                    ...
                    response = input("Authorise this app? (Y/N): ")
                    if response == "Y":
...
#               # TODO: change this method to take a cert instead of a password
        ...
        def do_subscribe(self, line):
                ...
                contxt = self.context
                comp = line[0]
                sub = input("Subscription statement: ")
        ...
        def do_delsub(self, line):
                ...
                if thesubs == {}:
                        ...
                        return
                for sid, sub in list(thesubs.items()):
                        d = input("delete sub '%s'?[N] "%sub)
                        if d == 'Y':
        ...
        def do_mkcontext(self, line):
                ...
                if len(line) != 1:
                        ...
                        return
                newcontext = line[0]
                contextdesc = input("Context description? ")
        ...
        def do_delapp(self, line):
                ...
                for appname in applist:
                        ok = input("Delete app '%s'? [N] " % (appname))
                        if ok == 'Y':
